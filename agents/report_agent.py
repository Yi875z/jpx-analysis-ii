...
- **相対取引の相手方**: ネット集計から取引の相手方は特定できない。
  「カウンターパーティ」「〜の反対側」と断定せず、「集計上、Aの買い越しに対しBが売り越し」とのみ表現する。
- **投資家の内訳・意図の断定禁止**:
  ...
  - 「保険付き」「本格的ポジション縮小」「一過性ではない」等、フローの目的・性質は断定しない。
    オプションデータには新規/手仕舞い・限月・行使価格・建玉増減が無いため、
    確認できるのは「現物・先物の売り越しとプット買い越しが同時に観測された」まで。
- **自己（証券会社）**: MMのヘッジ・在庫・顧客取引の反対売買であり、方向性ポジションと解釈しない。
  現物・先物の符号が一致しても両輪買い/両輪売りとは呼ばず、
  「現物・先物とも買い越し（ただしMM・在庫フロー）」と表現する。
- **GEX判定はProxy**: 本データには行使価格・限月・建玉が無い。「-GEX環境」と断定せず、
  「契約枚数ベースでは-GEXバイアスを示すProxy判定」のように推定であることを明記する。
- **統計閾値の規律**: 異常水準の基準は|Z|≧2.0のみ。Z=-3、「▼2兆円」等、根拠のない閾値を発明しない。
  セリングクライマックス等の相場イベント判定をZスコア単独で行わない
  （価格・出来高・ボラのデータが無い場合は「売りフローが統計的異常水準」までに留める）。
...
def _build_options_table(options_rows: list[dict]) -> str:
    ...
    for inv in OPTION_INVESTORS:
        ...
        lines.append(
            ...
            f"コール net {_std_eq(inv, 'call'):+,.1f}枚 / プット net {_std_eq(inv, 'put'):+,.1f}枚"
        )
...
- 海外投資家・自己・個人の **コール/プット別 net 枚数（生枚数と標準換算の両方）** をテーブルで提示
- **海外プット買い ≷ コール買い** の比較は**標準換算枚数**で行い、下方ヘッジ姿勢の強弱を判定
- **自己のプット net（標準換算）** から MM のガンマ・ポジションを **Proxy判定**として推定（±GEXバイアス表現）
